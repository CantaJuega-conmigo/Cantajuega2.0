...
export const ValidateOthersVideo = (
  FirstVideoProgress: Other_Video | Final_Video,
  lastVideoProgress: First_Video | Other_Video | Final_Video
) => {
  if (lastVideoProgress.Total > 3 && !lastVideoProgress.one_Day_Passed) {
    alert(
      "Ups aun no puedes avanzar, debe pasar un dia despues de ver el video anterior"
    );
    return false;
  }
